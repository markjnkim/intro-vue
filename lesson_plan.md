...
Let's practice creating by creating a link tag
...
Once again we will use the Vue directive.
...
* Call on students to answer the following questions one at time, filling in gaps in their explanations as necessary:
  ...
  * **Q:** What is a 2-way data bind?
    **A:** Data that is able to flow to and from the view.
  * **Q:** How do we set a click event listener on an element in Vue?
    **A:** We use the directive `v-on:click`.

### 2. Students Do: To Do List (20 mins) (Critical)
...
* Swap out your application's `src` folder with [src](../../../../01-Class-Content/19-react/01-Activities/30-Ins_FormsDemo/src). Stop the dev server if it is already running. Start the app in dev mode by running `yarn start`.
